...
print(OS_revenue)
revenue = data.groupby('user_operating_system').revenue.sum()
cost = data.groupby('user_operating_system').cost.sum()
profit = revenue-cost
        
#%% Modeling
#initiate a h2o cluster and transforme the data into a h2o-specific frame. 
h2o.init(max_mem_size = "2G", nthreads=-1)
col_types=['numeric','categorical','categorical','numeric',
...
#set the columns that will be used as features and the ones we want to predict.
y = 'isRevenue'
...
scaled_imp=RF._model_json['output']['variable_importances']['scaled_importance']
ax.barh(y_pos, scaled_imp, align='center', color='green', ecolor='black')
ax.set_yticks(y_pos)
