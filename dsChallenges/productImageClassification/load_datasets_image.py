        ...
        try:
            if num_images < 800:
                ...
                if image_data.shape != (image_size, image_size, pixel_depth):
                    ...
                    image_data = misc.imresize(
                        image_data, (image_size, image_size))  # resize the image
...
def load_images(folder_name, data_folders, min_num_images_per_class, force=True):
    ...
    for folder in data_folders:
        ...
        current_dataset, current_label = load_image(
            folder_direct, min_num_images_per_class)
        if dataset is None:
...
valid_dataset, valid_label = load_images(
    valid_folder, os.listdir(valid_folder), 1)
train_dataset, train_label = load_images(
    train_folder, os.listdir(train_folder), 1)
