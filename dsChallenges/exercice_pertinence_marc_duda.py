...
with open("offres.txt") as f:
    ...
    while index<len(content):  #read the lines of the input
        if int(content[index])>=0: #the current value is greater or equal to zero all ok we go to the next line
            ...
            if not(stayStill):
                ...
                next = 0
                while (not(foundCandidate) and next<=5 and (index+next)<len(content)):#search suitable line among the four next
                    foundCandidate = int(content[index+next])>=0
                    next+=1
                next = next-1
                if not(foundCandidate):#a suitable line is not found, we take the "least bad" among the 5 consecutive lines
