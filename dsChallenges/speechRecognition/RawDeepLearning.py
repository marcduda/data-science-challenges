...
# Fit the model
model_dl.fit(np.reshape(X_train, (-1, dim_features*1)), y_binary_train, epochs=20,
             batch_size=1000, class_weight=class_weight_dl, validation_split=0.1)

# evaluate the model
scores = model_dl.evaluate(np.reshape(
    X_test, (-1, dim_features*1)), y_binary_test)
...
model_cnn.add(Conv1D(filters=10, kernel_size=20, strides=1,
...
# Fit the model
model_cnn.fit(X_train, y_binary_train, epochs=20, batch_size=1000,
              class_weight=class_weight_cnn, validation_split=0.1)
