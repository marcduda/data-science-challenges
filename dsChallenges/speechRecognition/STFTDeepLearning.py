...
# Fit the model
model_dl.fit(X_train, y_binary_train, epochs=20, batch_size=1000,class_weight = class_weight_dl)
...
model_cnn.add(Conv2D(filters=5, kernel_size=(20,1), strides=5, padding='same',data_format='channels_last', dilation_rate=1, activation='relu'
...
model_cnn.add(Conv2D(filters=10, kernel_size=(5,1), strides=1, activation='relu'))
...
# Fit the model
model_cnn1D.fit(np.reshape(X_train,(X_train.shape[0],X_train.shape[1],1)), y_binary_train, epochs=20, batch_size=1000,class_weight = class_weight_cnn, validation_split=0.1)#

# evaluate the model
scores_cnn1D = model_cnn1D.evaluate(np.reshape(X_test,(X_test.shape[0],X_test.shape[1],1)), y_binary_test, verbose=1)
print("\n%s: %.2f%%" % (model_cnn.metrics_names[1], scores_cnn[1]*100))  #, y_test

#predict the labels of the test set and print some metrics to compare it with the correct labels
predictions_cnn1D = model_cnn1D.predict(np.reshape(X_test,(X_test.shape[0],X_test.shape[1],1)))
