...
model_dl.fit(X_train, y_binary_train, epochs=20,
             batch_size=2000, class_weight=class_weight_dl)
...
model_cnn1D.fit(np.reshape(X_train, (X_train.shape[0], X_train.shape[1], 1)), y_binary_train,
                epochs=20, batch_size=1000, class_weight=class_weight_cnn1D, validation_split=0.1)

# evaluate the model
scores_cnn = model_cnn1D.evaluate(np.reshape(
...
# predict the labels of the test set and print some metrics
# to compare it with the correct labels
predictions_cnn = model_cnn1D.predict(np.reshape(
...
model_cnn.add(Conv2D(filters=5, kernel_size=(3, 20), strides=5, padding='same', data_format='channels_last', dilation_rate=1,
