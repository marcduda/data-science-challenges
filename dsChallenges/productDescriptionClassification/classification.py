...
train = pd.read_csv('train_data.csv', sep=';',
                    dtype={'Identifiant_Produit': int, 'prix': str, 'Produit_Cdiscount': str, 'Marque': str, 'Libelle': str, 'Description': str, 'Categorie1': str, 'Categorie2': str, 'Categorie3': str})
...
descriptionsNull = data['Description'].loc[data['Description']
                                           != data['Description']].shape
cdiscountNull = data['Produit_Cdiscount'].loc[data['Produit_Cdiscount']
                                              != data['Produit_Cdiscount']].shape
...
vectorizer_s = StemmedCountVectorizer(ngram_range=(
    1, 2), min_df=3, analyzer="word", stop_words=stopwords.words("french"), tokenizer=LemmaTokenizer())

# apply
data['Marque'].fillna(value='', inplace=True)
data['Text'] = data['Marque']+" "+data['Description']

X = data[['Text', 'Categorie3']].loc[data['Categorie3'].notnull(
) & data['Text'].notnull()].drop(['Categorie3'], axis=1).values.reshape((-1,))
y = data['Categorie3'].loc[data['Categorie3'].notnull(
) & data['Text'].notnull()].values.reshape((-1,))

X, y = shuffle(X, y, random_state=0)

X_, X_test, y_, y_test = train_test_split(
    X[:100000], y[:100000], test_size=.1, random_state=0)


X_train, X_valid, y_train, y_valid = train_test_split(
    X_, y_, test_size=.2, random_state=0)
#%%
...
#%%
X_to_predict = data[['Text', 'Categorie3']].loc[data['Categorie3'].isnull()].drop(
    ['Categorie3'], axis=1).values.reshape((-1,))
prediction_SVM = text_clf.predict(X_to_predict)
print("SVM part, metrics on test set:")
test_label_df = pd.DataFrame(data=prediction_SVM, columns=['label'])
test_id_df = data[['Identifiant_Produit', 'Categorie3']
                  ].loc[data['Categorie3'].isnull()].drop(['Categorie3'], axis=1)
