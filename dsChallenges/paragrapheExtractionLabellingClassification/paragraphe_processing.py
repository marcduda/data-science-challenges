...
for file in files:
    if os.path.getsize(directory+file) > 0:
        ocr_txt = pd.read_csv(directory+file, header=None,
                              delim_whitespace=True, quoting=csv.QUOTE_NONE, encoding='utf-8')
        ocr_txt.columns = ['page', 'x0', 'y0', 'x1', 'y1', 'word']
        ocr_txt['paragraphe'] = 0
        paragraphe = 0
        current_string = ""
        for i in range(1, len(ocr_txt)):
            word = re.sub(r'[^\w\s]', ' ', str(
                ocr_txt.iloc[i, ocr_txt.columns.get_loc('word')]))
...
list_all_paragraphe_filtered = [
    para for para in list_all_paragraphe if len(para) > 20]
...
full_stopwords = stopwords.words(
    "french")+["ils", "elles", "les", "leurs", "i", "ii", "vi", "iv", "iii"]
list_all_paragraphe_filtered = [' '.join(
    [stemmer.stem(y) for y in z.split(' ')]) for z in list_all_paragraphe_filtered]
...
tfidf_vectorizer = TfidfVectorizer(max_df=0.80, min_df=5,
                                   ...
                                   stop_words=full_stopwords, token_pattern='[^\d\W]{2,}')
...
sentences_train, sentences_test, y_train, y_test = train_test_split(
    sentences_classif, cluster_labels, test_size=0.15, random_state=42)
...
y = cluster_labels
X = {i: list_all_paragraphe_filtered[i]
     for i in range(len(list_all_paragraphe_filtered))}
...
def create_rnn(top_words, embedding_vector_length, max_review_length, batch_size):
    ...
    model.add(Embedding(top_words, embedding_vector_length,
                        input_length=max_review_length))
    model.add(SimpleRNN(batch_input_shape=(
        1, 1, embedding_vector_length), units=1))
    model.add(Dense(10, activation='relu'))
    model.compile(loss='categorical_crossentropy',
                  optimizer='adam', metrics=['logloss'])
    return model
...
def executeModel(str, X_train, y_train, X_test, y_test, top_words, embedding_vector_length, max_review_length, batch_size):
    if str == 'rnn':
        model = create_rnn(top_words, embedding_vector_length,
                           max_review_length, batch_size)
    elif str == 'lstm':
        model = create_lstm(top_words, embedding_vector_length,
                            max_review_length, batch_size)
    model.fit(X_train, y_train, epochs=3, batch_size=batch_size, verbose=1,
              validation_split=0.1, shuffle=False)  # ,class_weight = class_weight_dl
...
modelRNN = executeModel('rnn', X_train, y_train, X_test, y_test, top_words,
                        embedding_vector_length, max_paragraphe_length, batch_size)

modelLSTM = executeModel('lstm', X_train, y_train, X_test, y_test,
                         top_words, embedding_vector_length, max_paragraphe_length, batch_size)
